...
| ¿Hay transferencia negativa? (¿La DANN borra señal predictiva legítima?) | Re-entrenar el XGBoost baseline de Fase 1 sobre `z_post-DANN` y comparar `feature_importance` contra el de `X` original. Las features cuyo peso predictivo se derrumba desproporcionadamente son candidatas a transferencia negativa. Complementar con la lectura de longitudes de flechas del Local Biplot pre/post adaptación. *Nota: el diagnóstico con TabNet (M_agg) queda excluido de esta fase porque TabNet sólo se entrena en Fase 3; ver §5 y §7.* |
...
**Refuerzo desde la tesis:** **la tesis no aporta sobre adaptación de dominio.** La pieza que en una primera versión de este documento se propuso aquí (M-GCECDL) se descarta por la justificación de §1.3 — el M-GCECDL no es una herramienta de adaptación de dominio, es de fusión multimodal con reliability gating, y los problemas son estructuralmente distintos. Lo que sí puede aportar la tesis en Fase 2 es **diagnóstico empírico geométrico**: aplicar Local Biplot sobre los embeddings `z_pre-DANN` y `z_post-DANN` para visualizar si las flechas de features clave se acortan desproporcionadamente tras adaptar (señal de transferencia negativa). Esta auditoría se complementa con la comparación de `feature_importance` de XGBoost pre/post DANN. **TabNet no se usa en este diagnóstico**, porque su entrenamiento pertenece operacionalmente a Fase 3 (ver §2.2).
...
| 9–14 | F2: DANN/CDAN | DANN clásico + CDAN como refinamiento | **Diagnóstico complementario** (no sustitutivo, **sin TabNet** — TabNet pertenece a Fase 3): (i) extraer embeddings `z_pre-DANN` y `z_post-DANN`; (ii) aplicar Local Biplot a ambos para visualizar si las flechas de `cole_area_ubicacion` se acortan tras adaptar (evidencia de invariancia); (iii) re-entrenar XGBoost sobre `z_post-DANN` y comparar `feature_importance` contra el baseline de Fase 1 para detectar transferencia negativa. |
